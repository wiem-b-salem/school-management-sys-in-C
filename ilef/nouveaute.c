...
// Charger les nouveautés à partir du fichier
...
// Sauvegarder les nouveautés dans le fichier
...
void ajouterNouveaute(Nouveaute *nouveautes, int *nbNouveautes) {
    Nouveaute n;
    printf("Entrez l'ID de la nouveauté: ");
    scanf("%d", &n.id);
    printf("Entrez le type de nouveauté (événement, club, etc.): ");
    scanf("%s", n.type);
    ...
    printf("Nouveauté ajoutée et sauvegardée avec succès!\n");
}
...
void afficherToutesLesNouveautes(Nouveaute *nouveautes, int nbNouveautes) {
    int i ;
    if (nbNouveautes == 0) {
        printf("Aucune nouveauté à afficher.\n");
        return;
    }
...
// Modifier une nouveauté
void modifierNouveaute(Nouveaute *nouveautes, int nbNouveautes) {
    int id, found = 0;
    int i ;
    printf("Entrez l'ID de la nouveauté à modifier: ");
    scanf("%d", &id);

    for ( i = 0; i < nbNouveautes; i++) {
        if (nouveautes[i].id == id) {
            found = 1;
            printf("Nouveauté trouvée. Entrez les nouvelles informations.\n");
            printf("Type: ");
            ...
            printf("Nouveauté modifiée avec succès!\n");
            break;
        }
    }

    if (!found) {
        printf("Nouveauté avec l'ID %d non trouvée.\n", id);
    }
...
void supprimerNouveaute(Nouveaute *nouveautes, int *nbNouveautes) {
    int id, found = 0;
    int i , j ;
    printf("Entrez l'ID de la nouveauté à supprimer: ");
    scanf("%d", &id);
    ...
    for ( i = 0; i < *nbNouveautes; i++) {
        if (nouveautes[i].id == id) {
            ...
            (*nbNouveautes)--;
            printf("Nouveauté supprimée avec succès!\n");
            break;
        }
    }
...
// Gestion des nouveautés via un menu
void gestionNouveautes() {
    Nouveaute nouveautes[100];
    int nbNouveautes = chargerNouveautes(nouveautes);

    int choix;
    do {
        printf("=== Gestion des nouveautés ===\n");
        printf("1 - Ajouter une nouveauté\n");
        printf("2 - Afficher toutes les nouveautés\n");
        printf("3 - Modifier une nouveauté\n");
        printf("4 - Supprimer une nouveauté\n");
        printf("0 - Retour\n");
